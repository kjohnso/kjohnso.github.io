...
while datetime.datetime.now(pytz.timezone('US/Eastern')) < endtime:
    ...
    df['SubwayPct'] = [pd.to_numeric(x.strip().replace('%', '')) / 100 for x in
                       df['Subways: % Change From Prior Year Equivalent Day']]
    ...
    df['BusPct'] = [pd.to_numeric(x.strip().replace('%', '')) / 100 for x in
                    df['Buses: % Change From Prior Year Equivalent Day']]
    ...
    fig = px.line(df,
                  ...
                  title='<b>Subway and Bus Estimated Ridership ' + df.iloc[0, 0].strftime('%m/%d/%Y') + ' - ' + df.iloc[
                      -1, 0].strftime(
                      '%m/%d/%Y') + ' (Source: ' + "</b><a href='https://new.mta.info/coronavirus/ridership'>MTA</a>" + '<b>)</b>',
    ...
    fig.write_html('Final_index.html', include_plotlyjs='cdn')
